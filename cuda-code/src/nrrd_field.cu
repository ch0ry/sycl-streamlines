...
nrrd_field::nrrd_field(const std::string &filename) {
  // auto nrrd = std::unique_ptr<Nrrd,decltype((&nrrdNuke))>( nrrdNew(),
  // nrrdNuke );
  //    auto nrrd = std::unique_ptr<Nrrd,decltype((&nrrdNuke))>( nrrdNew(),
  //    nrrdNuke );
  ...
  std::vector<float4> padded(nx * ny * nz);
  for (size_t i = 0; i < padded.size(); ++i) {
    padded[i] = make_float4(rawData[i * 3 + 0], rawData[i * 3 + 1],
                            rawData[i * 3 + 2], 1.0f);
  }

  m_array.resize({nx, ny, nz});
  m_array.copy(padded.data());

  hid_t scale_dset = H5Dopen(file, "/scale", H5P_DEFAULT);
  float scale[3];
  H5Dread(scale_dset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, scale);
  m_scale = make_float3(scale[0], scale[1], scale[2]);
  ...
  m_offset = make_float3(0.0f, 0.0f, 0.0f);
}
